...
func TestGetProofByHash(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:       "no error",
			statusCode: http.StatusOK,
			body:       []byte(testdata.GetProofByHashResponseBody),
			wantResponse: &ct.GetProofByHashResponse{
				LeafIndex: 10,
				AuditPath: [][]byte{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			gotResp, gotHTTPData, gotErr := lc.GetProofByHash(testonly.MustB64Decode(testdata.LeafHash), testdata.TreeSize)
			if gotErrType := reflect.TypeOf(gotErr); gotErrType != test.wantErrType {
				t.Errorf("GetProofByHash(%s, %d): error was of type %v, want %v", testdata.LeafHash, testdata.TreeSize, gotErrType, test.wantErrType)
			}
			if gotHTTPData == nil {
				t.Fatalf("GetProofByHash(%s, %d) = (_, nil, _), want an HTTPData containing at least the timing of the request", testdata.LeafHash, testdata.TreeSize)
			}
			if gotHTTPData.Timing.Start.IsZero() || gotHTTPData.Timing.End.IsZero() {
				t.Errorf("GetProofByHash(%s, %d): HTTPData.Timing = %+v, want the Timing to be populated with the timing of the request", testdata.LeafHash, testdata.TreeSize, gotHTTPData.Timing)
			}
			if !bytes.Equal(gotHTTPData.Body, test.body) {
				t.Errorf("GetProofByHash(%s, %d): HTTPData.Body = %s, want %s", testdata.LeafHash, testdata.TreeSize, gotHTTPData.Body, test.body)
			}
...
func TestAddChain(t *testing.T) {
	var (
		// These are copies of testdata.SCT, but tweaked so that they are missing fields.
