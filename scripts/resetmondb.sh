...
- MYSQL_ROOT_USER: A user with sufficient rights to create/reset the
  Monologue database (default: root).
...
- MYSQL_DATABASE: The name to give to the new Monologue user and database
  (default: monologuetest).
- MYSQL_USER: The name to give to the new Monologue user
  (default: monologuetest).
- MYSQL_PASSWORD: The password to use for the new Monologue user
  (default: soliloquy).
- MYSQL_USER_HOST: The host that the Monologue user will connect from;
  use '%' as  a wildcard (default: localhost).
EOF
}
...
collect_vars() {
  ...
  [ -z ${MYSQL_DATABASE+x} ] && MYSQL_DATABASE="monologuetest"
  [ -z ${MYSQL_USER+x} ] && MYSQL_USER="monologuetest"
  [ -z ${MYSQL_PASSWORD+x} ] && MYSQL_PASSWORD="soliloquy"
...
main() {
  ...
  readonly INCIDENT_PATH=$(go list -f '{{.Dir}}' github.com/google/monologue/incident/mysql)
  ...
  then
      ...
      mysql "${FLAGS[@]}" -D ${MYSQL_DATABASE} < ${INCIDENT_PATH}/incident.sql || \
        die "Error: Failed to create incident table in '${MYSQL_DATABASE}' database."
      echo "Reset Complete"
  fi
}
